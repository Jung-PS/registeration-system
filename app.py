...
def log(ip,agent,data):
    with open("log.txt", "a") as file:
        file.write(f"{datetime.now(timezone).strftime('%d-%m-%Y %H:%M:%S')} {ip} {agent} {data}\n")
@app.route("/")
def home():
    try:
...
def register():
    if request.method == "POST":
        ...
        try:
            log(request.remote_addr,request.headers.get('User-Agent'),f"Register {username} {prefix}{full_name} {school_name} {school_grade}")
        except:
            log(request.remote_addr,request.headers.get('User-Agent'),f"Register {username} {prefix}{full_name} {school_name} {school_grade}")
...
def login():
    if request.method == "POST":
        ...
        if len(password) > 100:
            flash("Malformed request", "error")
            try:
                log(request.headers['X-Forwarded-For'], request.headers.get('User-Agent'), f"Login {username} (Failed Malformed)")
            except:
                log(request.remote_addr,request.headers.get('User-Agent'),f"Login {username} (Failed Malformed)")
            return redirect(url_for("login"))
        ...
        if username in users:
            ...
            if bcrypt.checkpw(password.encode('utf-8'), stored_hash):
                ...
                try:
                    log(request.headers['X-Forwarded-For'], request.headers.get('User-Agent'), f"Login {username} (Success)")
                except:
                    log(request.remote_addr,request.headers.get('User-Agent'),f"Login {username} (Success)")
                return redirect(url_for("home"))
            else:
                flash("Invalid username or password", "error")
                try:
                    log(request.headers['X-Forwarded-For'], request.headers.get('User-Agent'), f"Login {username} (Failed)")
                except:
                    log(request.remote_addr,request.headers.get('User-Agent'),f"Login {username} (Failed)")
        else:
            flash("Invalid username or password", "error")
            try:
                log(request.headers['X-Forwarded-For'], request.headers.get('User-Agent'), f"Login {username} (Failed)")
